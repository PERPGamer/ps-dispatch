...
function GetPlayerGender()
    ...
    if ESX.GetPlayerData().gender == 1 then
...
function GetIsHandcuffed()
    return ESX.GetPlayerData().IsArrested
end

function IsOnDuty()
    if Config.OnDutyOnly then
        if ESX.GetPlayerData().job.onduty then
            return true
        else
            return false
        end
...
function IsCallAllowed(message)
    ...
    if Config.PhoneRequired and not HasPhone() then ESX.ShowNotification('You need a communications device for this.', 'error', 5000) return false end
